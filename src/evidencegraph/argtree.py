...
class RelationSet(object):
    ...
    def __init__(self, preserving, inverting):
        ...
        self.functions = ([self.function_central_claim] +
...
SIMPLE_RELATION_SET = RelationSet(
    ['sup'], ['att'])
...
class ArgTree(nx.DiGraph):
    ...
    def __init__(self, from_arggraph=None, from_triples=None, text_id=None,
        ...
        if from_arggraph and from_triples:
            raise Exception(("Cannot load argtree from arggraph and from "
    ...
    def load_from_arggraph(self, g, from_adus=True, long_names=False):
        ...
        r = g.get_edus_as_dependencies(
            include_cc=False, ids_to_numbers=True)
        if from_adus:
            r = self.edu_triples_to_adu_triples(r)
        # reduce to relation set
        reduced = {
            'exa': 'sup', 'add': 'sup', 'link': 'sup',
            'join': 'sup', 'restate': 'sup',
            'reb': 'att', 'und': 'att',
        }
        ...
        longer = {
            'sup': 'support', 'exa': 'example',
            'reb': 'rebut', 'und': 'undercut', 'att': 'attack'}
        if long_names:
    ...
    def get_vector(self):
        ...
        """
        return {'cc': self.get_cc_vector(), 'ro': self.get_ro_vector(),
                'fu': self.get_fu_vector(), 'at': self.get_at_vector()}
