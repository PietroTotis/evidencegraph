...
def evaluate(ground_truth, prediction):
    ...
    ('classwise', {'0': {'precision': 0.5, 'recall': 1.0, 'fscore': 0.6666666666666666}, \
    '1': {'precision': 0.0, 'recall': 0.0, 'fscore': 0.0}, \
    '2': {'precision': 0.75, 'recall': 1.0, 'fscore': 0.8571428571428571}}), \
    ('confusions', {0: {0: 1, 1: 0, 2: 0}, 1: {0: 1, 1: 0, 2: 1}, 2: {0: 0, 1: 0, 2: 3}}), \
    ('k_cohen', {'k': 0.45454545454545453, 'AE': 0.38888888888888884, 'AO': 0.6666666666666666}), \
    ('k_fleiss', {'k': 0.41463414634146323, 'AE': 0.43055555555555564, 'AO': 0.6666666666666666}), \
    ('macro_avg', {'precision': 0.4166666666666667, 'recall': 0.6666666666666666, 'fscore': 0.5079365079365079}), \
    ('micro_avg', {'precision': 0.6666666666666666, 'recall': 0.6666666666666666, 'fscore': 0.6666666666666666}), \
    ('true_cat_dist', [1, 2, 3]), \
    ('weigh_avg', {'precision': 0.4583333333333333, 'recall': 0.6666666666666666, 'fscore': 0.5396825396825397})]
    ...
    confusions = {gold: {pred: 0 for pred in categories}
    ...
    results['macro_avg'] = prfs_to_dict(precision_recall_fscore_support(
            ...
            average='macro', pos_label=None, warn_for=()))
    results['micro_avg'] = prfs_to_dict(precision_recall_fscore_support(
            ground_truth, prediction,
            average='micro', pos_label=None, warn_for=()))
    results['weigh_avg'] = prfs_to_dict(precision_recall_fscore_support(
            ground_truth, prediction,
            average='weighted', pos_label=None, warn_for=()))
    ...
    expected_agreement_cohen = sum([
        (gold_category_distribution[c] / items_count) *
        (pred_category_distribution[c] / items_count)
        for c in categories
    ])
...
def eval_prediction(gold_trees, pred_trees):
    ...
    """
    assert [t.get_tid() for t in gold_trees] == [t.get_tid() for t in pred_trees]
    ...
    def score_it(gold, pred, level):
        ...
        assert len(level_gold) == len(level_pred), "{} {} {}".format(level, level_gold, level_pred)
...
def load_predictions(path, replace_rel=None, relation_set=FULL_RELATION_SET):
    ...
    for iteration, texts in raw.items():
        for tid, triples in texts.items():
            if replace_rel:
                triples = [(s, t, replace_rel.get(r, r))
                           for s, t, r in triples]
            t = ArgTree(from_triples=triples, text_id=tid,
