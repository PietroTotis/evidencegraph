...
class ArgGraph (nx.DiGraph):
    ...
    def to_xml(self):
        ...
        for node in sorted_nicely(self.get_adus()):
                ...
                new_ids[node] = adu_id
                max_adu_count += 1
                adu_elm = etree.XML('<adu id="{}" type="{}" />'.format(
                    adu_id, adu_type))
        ...
        for source, target, data in sorted(self.edges(data=True)):
            ...
            if (edge_type == 'seg' and
                    node_types in [('edu', 'adu'), ('edu', 'joint'),
                                   ('joint', 'adu')]):
                edge_id = 'c%d' % max_edge_count
                max_edge_count += 1
                edge_elm = etree.XML(edge_xml.format(
        ...
        while not q.empty():
            ...
            sources = sorted_nicely(
                [source for source in self.predecessors(node)
                 if (self.node[source]['type'] == 'adu' and
                     self.edge[source][node]['type'] == 'src')])
            # first source
            source = sources[0]
        ...
        return etree.tostring(doc_elm, encoding='UTF-8', pretty_print=True,
    ...
    def show_in_ipynb(self, edu_cluster=False):
        ...
        dot = str(self.export_to_dot(edu_cluster=edu_cluster))
    ...
    def get_adu_segmented_text(self):
        ...
        """
        return [' '.join([text for _, text in self.get_edus_of_adu(adu)])
    ...
    def get_edus_as_dependencies(self, include_cc=False, ids_to_numbers=False):
        ...
        adus = self.get_adus()
        for adu in adus:
            edus = [i for i in self.predecessors(adu)
        ...
        for adu in adus:
            edus = sorted_nicely([
                head[i] for i in self.predecessors(adu)
        ...
        while not q.empty():
            ...
            if target not in head:
                ...
                continue
            edus = sorted_nicely([
                head[i] for i in self.predecessors(node)
...
def get_minimal_arggraph():
    ...
    >>> a.node
    {'e1': {'type': 'edu', 'text': 'Swim!'}, 'a1': {'type': 'adu', 'role': 'pro'}, 'e2': {'type': 'edu', 'text': 'Good weather.'}, 'a2': {'type': 'adu', 'role': 'pro'}, 'c1': {'type': 'rel'}}
    >>> a.edge
    {'e1': {'a1': {'type': 'seg'}}, 'a1': {}, 'e2': {'a2': {'type': 'seg'}}, 'a2': {'c1': {'type': 'src'}}, 'c1': {'a1': {'type': 'sup'}}}
    """
    a = ArgGraph(id='g1')
...
def get_simple_arggraph():
    ...
    >>> a.node
    {'e1': {'type': 'edu', 'text': 'Swim!'}, 'a1': {'type': 'adu', 'role': 'pro'}, 'e2': {'type': 'edu', 'text': 'Good weather.'}, 'a2': {'type': 'adu', 'role': 'pro'}, 'c1': {'type': 'rel'}, 'e3': {'type': 'edu', 'text': 'Sharks!!!!1!'}, 'a3': {'type': 'adu', 'role': 'opp'}, 'c2': {'type': 'rel'}, 'e4': {'type': 'edu', 'text': 'Bought anti-sharks-spray.'}, 'a4': {'type': 'adu', 'role': 'pro'}, 'c3': {'type': 'rel'}}
    >>> a.edge
    {'e1': {'a1': {'type': 'seg'}}, 'a1': {}, 'e2': {'a2': {'type': 'seg'}}, 'a2': {'c1': {'type': 'src'}}, 'c1': {'a1': {'type': 'sup'}}, 'e3': {'a3': {'type': 'seg'}}, 'a3': {'c2': {'type': 'src'}}, 'c2': {'c1': {'type': 'und'}}, 'e4': {'a4': {'type': 'seg'}}, 'a4': {'c3': {'type': 'src'}}, 'c3': {'c2': {'type': 'und'}}}
    """
    a = ArgGraph(id='g1')
    a.add_edu_adu('e1', 'Swim!', 'a1', 'pro')
    a.add_edu_adu('e2', 'Good weather.', 'a2', 'pro')
...
def get_complex_arggraph():
    ...
    >>> a = get_complex_arggraph()
    >>> a.node
    {'E1': {'type': 'edu', 'text': 'Swim!'}, 'A1': {'type': 'adu', 'role': 'pro'}, 'E2': {'type': 'edu', 'text': 'Good weather.'}, 'A2': {'type': 'adu', 'role': 'pro'}, 'C1': {'type': 'rel'}, 'J1': {'type': 'joint'}, 'E3': {'type': 'edu', 'text': 'Sharks!'}, 'E3.1': {'type': 'edu', 'text': '!!!1!'}, 'A3': {'type': 'adu', 'role': 'opp'}, 'C2': {'type': 'rel'}, 'E4': {'type': 'edu', 'text': 'Bought anti-sharks-spray.'}, 'A4': {'type': 'adu', 'role': 'pro'}, 'C3': {'type': 'rel'}, 'E5': {'type': 'edu', 'text': 'It is effective.'}, 'A5': {'type': 'adu', 'role': 'pro'}, 'E6': {'type': 'edu', 'text': 'So let us swim!'}}
    >>> a.edge
    {'E1': {'A1': {'type': 'seg'}}, 'A1': {}, 'E2': {'A2': {'type': 'seg'}}, 'A2': {'C1': {'type': 'src'}}, 'C1': {'A1': {'type': 'sup'}}, 'J1': {'A3': {'type': 'seg'}}, 'E3': {'J1': {'type': 'seg'}}, 'E3.1': {'J1': {'type': 'seg'}}, 'A3': {'C2': {'type': 'src'}}, 'C2': {'C1': {'type': 'und'}}, 'E4': {'A4': {'type': 'seg'}}, 'A4': {'C3': {'type': 'src'}}, 'C3': {'C2': {'type': 'und'}}, 'E5': {'A5': {'type': 'seg'}}, 'A5': {'C3': {'type': 'src'}}, 'E6': {'A1': {'type': 'seg'}}}
    """
    a = ArgGraph(id='g1')
    a.add_edu_adu('E1', 'Swim!', 'A1', 'pro')
    a.add_edu_adu('E2', 'Good weather.', 'A2', 'pro')
