...
class BaseClassifier(object):
    ...
    def train(self, in_data, gold_data):
        ...
        features = [
            feature for datum in in_data
            for feature in self.extract_features(datum)]
        labels = [
            label for datum in gold_data
            for label in self.extract_labels(datum)]
        self._train(features, labels)

    def train_optimize(self, in_data, gold_data, verbose=False):
        self.pipeline = GridSearchCV(
            self.pipeline, BaseClassifier.DEFAULT_PARAM_SEARCH,
...
class EvidenceGraphClassifier(object):
    ...
    def _extend_probas_to_all_classes(self, probas):
        ...
        fu_classes_all = sorted(self.relation_set.map_function_to_vector.values())
    ...
    def score(self, input_tree, output_tree):
        ...
        for level in self.ensemble.keys():
            _, _, macro_f1, _ = precision_recall_fscore_support(
                gold[level], pred[level], average='macro', pos_label=None,
                warn_for=())
            score *= macro_f1
        return score
        ...
        for (source, target), p_at in itemized_predictions['at'].items():
            ...
            for func_type in map_fu_to_vec.keys():
                ...
                if func_type in self.relation_set.functions_inverting_role:
                    ro_weight = (p_ro_source[0] * p_ro_target[1] +
                                 p_ro_source[1] * p_ro_target[0])
                else:
                    ro_weight = (p_ro_source[0] * p_ro_target[0] +
